...
class MotorClass {
    public:
        ...
        MotorClass(): 
        test_encoders_(false),reset_odom_(false),encoder_limit_(3000),leftSpeed_(128),rightSpeed_(128), rec_leftSpeed_(128),rec_rightSpeed_(128), PI_(3.1415926)
        {
            std::cout<<"I AM IN"<<std::endl;
            ros::NodeHandle nh;
            nh.getParam("MD49device", motor_device_);
            nh.getParam("wheel_separation", wheel_separation_);
            nh.getParam("wheel_radius", wheel_radius_);
            nh.getParam("ticks_per_rotation", ticks_per_rotation_);
            nh.getParam("ODOM_RATE", odom_rate_);
            nh.getParam("ACC_RATE", acc_rate_);
            ...
            resetOdomSrv = nh.advertiseService("/gobot_motor/resetOdom", &MotorClass::resetOdom, this);
            resetEncodersSrv = nh.advertiseService("/gobot_motor/resetEncoders", &MotorClass::resetEncoders, this);

            odom_pub_ = nh.advertise<nav_msgs::Odometry>("/odom", 50);
            odom_test_pub_ = nh.advertise<gobot_msg_srv::OdomTestMsg>("/odom_test", 50);
            encoder_pub_ = nh.advertise<gobot_msg_srv::EncodersMsg>("/encoders", 50);
            real_vel_pub_ = nh.advertise<geometry_msgs::Twist>("/real_vel", 50);
        ...
        bool initSerial(){
            ...
            ROS_INFO("(wheels::initSerial) MD49 port : %s", motor_device_.c_str());
        ...
        void odomThread(){
            ...
            if(encoders.size() == 8){
                ...
                if(abs(delta_left_encoder/dt)>encoder_limit_|| abs(delta_right_encoder/dt)>encoder_limit_){
                    ...
                    double left_dist = (delta_left_encoder / ticks_per_rotation_) * 2.0 * wheel_radius_ * PI_;
                    double right_dist = (delta_right_encoder / ticks_per_rotation_) * 2.0 * wheel_radius_ * PI_;
                    ...
                    double vx = vel * cos(odom_th_);
                    double vy = vel * sin(odom_th_);
    ...
    private:
        ...
        bool test_encoders_, reset_odom_;
        int acc_rate_,rolling_window_size_, encoder_limit_, odom_rate_;
        double odom_x_, odom_y_, odom_th_ , PI_ , wheel_separation_, wheel_radius_ ,ticks_per_rotation_;
