...
uint8_t wantedLeftSpeed = 128;
uint8_t wantedRightSpeed = 128;
float currentLeftSpeed = 128.0;
float currentRightSpeed = 128.0;
int rightSpeedDiff;
int leftSpeedDiff;
bool test = false;

/// Handle the acceleration of the robot
void setSpeedsThread(void){
    std::cout << "(wheels::setSpeedsThread) Thread launched" << std::endl;
    ros::Rate r(20);

    uint8_t accelerationStep = 20;

    while(ros::ok()){
        if(currentRightSpeed != wantedRightSpeed || currentLeftSpeed != wantedLeftSpeed){
            if(wantedLeftSpeed == 128 && wantedRightSpeed == 128){
                currentRightSpeed = 128;
                currentLeftSpeed = 128;
            } else {
                if(currentRightSpeed != wantedRightSpeed)
                    currentRightSpeed += (wantedRightSpeed < currentRightSpeed) ? std::max(rightSpeedDiff / (float) accelerationStep, wantedRightSpeed - currentRightSpeed)
                                                                                : std::min(rightSpeedDiff / (float) accelerationStep, wantedRightSpeed - currentRightSpeed);
                if(currentLeftSpeed != wantedLeftSpeed)
                    currentLeftSpeed += (wantedLeftSpeed < currentLeftSpeed) ? std::max(leftSpeedDiff / (float) accelerationStep, wantedLeftSpeed - currentLeftSpeed)
                                                                             : std::min(leftSpeedDiff / (float) accelerationStep, wantedLeftSpeed - currentLeftSpeed);
            }

            writeAndRead(std::vector<uint8_t>({0x00, 0x31, static_cast<uint8_t>(std::round(currentLeftSpeed)), 0x00, 0x32, static_cast<uint8_t>(std::round(currentRightSpeed))}));
            std::cout << "(wheels::setSpeedsThread) Wanted speeds : " << (int) wantedLeftSpeed - 128 << " " << (int) wantedRightSpeed - 128 << " vs current speeds : " << currentLeftSpeed - 128 << " " << currentRightSpeed - 128 << std::endl;
        }
...
std::vector<uint8_t> writeAndRead(std::vector<uint8_t> toWrite, int bytesToRead){
    ...
    if(serialConnection.isOpen()){
        ...
        size_t bytesWrote = serialConnection.write(toWrite);
...
bool setSpeeds(gobot_base::SetSpeeds::Request &req, gobot_base::SetSpeeds::Response &res){
    wantedLeftSpeed = req.directionL.compare("F") == 0 ? 128 - req.velocityL : 128 + req.velocityL;
    wantedRightSpeed = req.directionR.compare("F") == 0 ? 128 - req.velocityR : 128 + req.velocityR;


    rightSpeedDiff = wantedRightSpeed - currentRightSpeed;
    leftSpeedDiff = wantedLeftSpeed - currentLeftSpeed;
