...
bool cliff_on=false, bumper_on=false;
bool moved_from_cliff = true;
...
void cliffCallback(const gobot_msg_srv::CliffMsg::ConstPtr& cliff){
    if((cliff->cliff1 > cliff_threshold) || (cliff->cliff2 > cliff_threshold) || (cliff->cliff3 > cliff_threshold) || (cliff->cliff4 > cliff_threshold)){
        moved_from_cliff = false;
...
void newCmdVel(const geometry_msgs::Twist::ConstPtr& twist){
    ...
    if(!collision && !pause_robot && moved_from_cliff){
