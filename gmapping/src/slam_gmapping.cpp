...
SlamGMapping::SlamGMapping():
    ...
    laser_count_(0), private_nh_("~"), scan_filter_sub_(NULL), scan_filter_(NULL), transform_thread_(NULL), footprint()
{
    seed_ = time(NULL);
    init();
}
...
SlamGMapping::SlamGMapping(ros::NodeHandle& nh, ros::NodeHandle& pnh):
    map_to_odom_(tf::Transform(tf::createQuaternionFromRPY( 0, 0, 0 ), tf::Point(0, 0, 0 ))),
    laser_count_(0),node_(nh), private_nh_(pnh), scan_filter_sub_(NULL), scan_filter_(NULL), transform_thread_(NULL), footprint()
{
    seed_ = time(NULL);
    init();
}
...
SlamGMapping::SlamGMapping(long unsigned int seed, long unsigned int max_duration_buffer):
    ...
    seed_(seed), tf_(ros::Duration(max_duration_buffer)), footprint()
{
    init();
}
...
void SlamGMapping::init()
{
    ...
    if(!private_nh_.getParam("use_footprint", use_footprint))
        use_footprint = false;
    else
        ROS_INFO("Got use_footprint : %d", use_footprint);

    /// We get the footprint as a XmlRpcValue
    XmlRpc::XmlRpcValue xml_footprint;
    if(private_nh_.getParam("footprint", xml_footprint))
        ROS_ASSERT(footprint.init(xml_footprint));
    else 
        use_footprint = false;
}
...
SlamGMapping::laserCallback(const sensor_msgs::LaserScan::ConstPtr& scan)
{
    ...
    if(addScan(*scan, odom_pose))
    {
        ...
        if(use_footprint && got_map_ && (scan->header.stamp - last_map_update) > ros::Duration(map_update_interval_.toSec() / (double) 5))
            footprint.updateFootprint(map_, map_frame_, scan->header.stamp, base_frame_);
...
SlamGMapping::updateMap(const sensor_msgs::LaserScan& scan)
{
    ...
    if(use_footprint){
        if(!got_map_)
            footprint.updateFootprint(map_, map_frame_, scan.header.stamp, base_frame_);
        footprint.addFootprintToMap(map_);
    }
