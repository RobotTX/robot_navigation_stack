...
double cov_xy=0.0,cov_yaw=0.0,initial_cov_xy=0.3,initial_cov_yaw=2.0, home_cov_xy=0.15,home_cov_yaw=0.1;
...
bool evaluatePose(int type){
    ...
    if(type==0){
        if((std::abs(rosparam_cov_x)+std::abs(rosparam_cov_y)) < COV_XY_T && std::abs(rosparam_cov_yaw)<COV_YAW_T)
            return true;
        else
            return false;
    }
...
bool rotateFindPose(double rot_v,double rot_t){
    ...
    while(running && dt<rot_t && (cov_xy>COV_XY_T || cov_yaw>COV_YAW_T)){
...
bool initializePoseSrvCallback(std_srvs::Empty::Request &req, std_srvs::Empty::Response &res){
    if(!running){
        std::thread([](){
            ...
            while(current_state!=COMPLETE_STATE && running){
