...
// checks if the estimated position published by amcl is good enough (actually it checks if the covariance is low which only means that amcl 
// is pretty sure about its guess although it might still be wrong)
