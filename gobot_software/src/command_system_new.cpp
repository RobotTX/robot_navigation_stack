...
bool goToChargingStation(const std::vector<std::string> command){
    if(command.size() == 1){
        ...
        else if(get_dock_status.response.data==3){
...
bool keyboardControl(const std::vector<std::string> command){
    if(command.size() == 1) {
        ...
        if(!keyboardEnabled){
            cmd = "gnome-terminal -x bash -c \"source /opt/ros/kinetic/setup.bash;source /home/"+ user_name + "/catkin_ws/devel/setup.bash;rosrun teleop_twist_keyboard teleop_twist_keyboard.py\"";
...
void sendConnectionData(boost::shared_ptr<tcp::socket> sock){
    ...
    gobot_msg_srv::GetStringArray get_home, get_speed;
    ...
    std::string looping_str = (get_loop.response.data) ? "1" : "0";
...
int main(int argc, char* argv[]){
    ...
    try{
        ...
        if(n.hasParam("user_name")){
            ...
            ROS_INFO("(Command system) user name : %s", user_name.c_str());
        }
        ...
        getDockStatusSrv = n.serviceClient<gobot_msg_srv::GetInt>("/gobot_status/get_dock_status");
